...
class WindowCapture:
    ...
    def convertPILToCv2(self, pil_img:Image) -> np.ndarray:
        """
        Converts PIL Image to a img that can be used by opencv2
        """
        open_cv_image: np.ndarray = np.array(pil_img)
        # Convert RGB to BGR
    ...
    def forceMinimised(self) -> None:
        ctypes.windll.user32.ShowWindow(self.hwnd, win32con.SW_SHOWMINIMIZED)

    def forceMaximised(self) -> None:
        ctypes.windll.user32.ShowWindow(self.hwnd, win32con.SW_SHOWMAXIMIZED)
    
    def forceNotActive(self) -> None:
        ctypes.windll.user32.ShowWindow(self.hwnd, win32con.SW_SHOWNOACTIVATE)

    def get_screenshot(self, addedX=0, addedY=0) -> np.ndarray:
        """
        Uses PrintWindow to capture a screenshot of specific window as this is more reliable
        """
    ...
    def get_screenshotOLD(self) -> np.ndarray:
        """
        Old version of taking a screenshot of a specific window using bitmap
        """
        # Check that window is not minimized as some parts of WallpaperGaming require it not to be. While others don't matter
        if self.checkVisibility(self.hwnd)[0] == True:
            print("Please don't minimize window.")
            # ctypes.windll.user32.ShowWindow(self.hwnd, 6)
            self.forceNotActive()
    ...
    def list_window_names() -> None:
        """
        Prints out all available windows and their handles
        """
    ...
    # WARNING: if you move the window being captured after execution is started it will return incorrect coords
    # pos = (x, y)
    def get_screen_position(self, pos:tuple) -> tuple:
