...
class WallpaperGaming():
    ...
    def __init__(self, gameWindowName, extraX=0, extraY=0, showCV2Window=False) -> None:
        ...
        if self.BEFORE_BG != None:
            print('Default background path:', self.BEFORE_BG)
        else:
            print("Warning: Default background path couldn't be found")
    ...
    def keyPressed(self, a:keyboard.KeyboardEvent) -> None:
        """
        Presses a virtual keypress onto specific window
        """
        if self.EXITED:
            return
        ...
        def code(self:WallpaperGaming, a:keyboard.KeyboardEvent) -> str | None:
            keyname = a.name
    ...
    def addText(self, img, TEXT, xy=(100,20), BGR=(0,0,255), thickness=1, fontScale=1) -> None:
        """
        Add text to cv2 window.
        """
    ...
    def addCv2WindowInfo(self, frame, processTime, RGB=(255,0,0)) -> None:
        """
        Adds main info text to cv2 window
        """
        offset = 40
    ...
    def toggleCv2(self, setTo=None) -> None:
    ...
    def start(self, showCV2Window=False, showFPS=False, FPSLabel=None, extraX=0, extraY=0, showAlertBoxes=True, exitKeyPressDetect=False, ignoreErrors=False) -> None:
        """
        Starts running main WallpaperGaming script
        """
        if self.EXITED:
            return
        ...
        try:
            if self.showAlertBoxes:
                root = tk.Tk()
                root.after(150, messagebox.showinfo, ('Wallpaper Gaming', 'Wallpaper Gaming : start(). Has finished!'))
                root.after(10000, root.destroy)
                root.mainloop()
                
        except RuntimeError:
            print('Wallpaper Gaming : start(). Has finished! However while doing so it ran into a error for displaying messagebox (Presumally because root window closed before it could run this)')
        
    def exit(self, root:tk.Tk) -> None:
        """
        Exits WallpaperGaming (and uses sys.exit(0))
        """
        self.EXITED = True
        try:
